...
One of the most challenging aspects of prototyping new programming langugages is to define their dynamic semantics.
A popular approach is to define the dynamic semantics as an interpreter in a different language \cite{reynolds-interpreters}.
...
One problem arises when trying to extend such language definitions.
...
<
< effect Env where
<   withInsert : String -> Int -> a ! Env, D -> a ! Env, D
<   lookup : String -> Int ! Env, D
<
< eval_Let : L_Let => Int ! Env, D
< eval_Let (Let v mx my) = do
<   x <- mx
<   withInsert v x my
< eval_Let (Var v) = lookup v
<
...
Now the |eval_Arith| algebra does not have to mention environments.



% NEW STORY: We want to define the semantics of our programming language elegantly using effects,
% but running such interpreters is slow because of the inherent interpretation overhead.
% In this paper, we explore an approach to compiling effects to efficient machine code.
% 
% S:
% We want to give semantics to programming languages in a modular way using effect handlers.
% Running programs then involves running an interpreter over those effects with associated computational overhead.
% 
% C:
% 
% Q:
